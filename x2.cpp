  ...
  // float density = 0.1;
    ...
    for(; density < 1.02; density+=0.05)
    {
      ...
      if(Ih == 8)
        ...
        // Space for CSCC:
        s_csr = (Adata.size() + Aindices.size() + Aindptr.size());
      ...
      transform2dTo1dv1(IN, DA, ptr, IN_1d_v7, DA_1d_v7, ptr_1d_v7);
            ...
            //CSR:
        {
            ...
            if(n != 1)
            {
                ...
                // copy data to double
              std::vector<double> DA_1d_v7_d;            
              std::vector<int> IN_1d_v7_d;
              std::vector<int> ptr_1d_v7;

                // CSR1(DA_1d_v7_d, IN_1d_v7_d, ptr_1d_v7, org_fm, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw);

              
              CSR2(DA_1d_v7_d, IN_1d_v7_d, ptr_1d_v7, org_fm, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw);

              // if(isErrorCSRPtr(Aindptr, ptr_1d_v7)) 
              // {
              //   cout << "XXXX Error CSR Ptr" << endl;
              //   exit(0);
              // }
              // else if(isErrorCSRInd(Aindices, IN_1d_v7_d, Adata, DA_1d_v7_d))
              // {
              //   cout << "XXXX Error CSR Data" << endl;
              //   exit(0);
              // }
              // else
              // {
