...
int check2Vectors(std::vector<int>& x, std::vector<int>& y)
{
    if (x.size() != y.size())
    {
        cout << "Two vectors are NOT Equal in size!" << endl;
        return 0;
    }
    for (int i = 0; i < x.size(); ++i)
    {
        if (x[i] != y[i])
        {
            cout << "Two vectors are NOT Equal in values!" << endl;
            return 0;
        }
    }
    return 1;
...
void CPS_if(int Ih, int j, int i, int nnz, int &prefix, int set_idx,  vector<int>& idx_2, vector<int>& IN_1d)
{
    if (nnz > 2)
    {
        // push(IN[l][ml-1]) <- {prefix,set_idx}
        IN_1d.insert(IN_1d.end(), { j * Ih + i - (i % 4) , set_idx});
        prefix = j * Ih + i - (i % 4);
        // cout << "nnz > 2" << endl;
        // printVector(IN_1d);
        // IN_1d.push_back(prefix);
        // IN_1d.push_back(set_idx); 
    }
    else if (nnz < 3)
    {
        // push(IN[l][ml-1]) <- {prefix,(j*Ih + i) }
        // IN_1d.insert(IN_1d.end(), { prefix, (j*Ih + i)}); 
        // IN_1d.insert(IN_1d.end(), { prefix, idx_2}); 
        IN_1d.insert(IN_1d.end(), idx_2.begin(), idx_2.end()); 
        // cout << "nnz < 3 && idx_2 -->" << endl;
        // cout << "idx_2 -->  " << endl;
        // printVector(idx_2);
        // printVector(IN_1d);
        // IN_1d.push_back(prefix);
        // IN_1d.push_back((j*Ih + i)); 
    }
    // else if (nnz == 1)
    // {
        // push(IN[l][ml-1]) <- {prefix}
        // IN_1d.insert(IN_1d.end(), { prefix });

        // IN_1d.insert(IN_1d.end(), idx_2.begin(), idx_2.end()); 
        // IN_1d.insert(IN_1d.end(), { idx_2 }); 
...
void CPS_idx(int Ih, int j, int i, int &nnz, int &prefix, int &set_idx,  vector<int>& idx_2, vector<int>& IN_1d)
{
    if ((i%4) == 3)
    {
        // cout << "(i%4) == 3" << endl;
        CPS_if(Ih, j, i, nnz, prefix, set_idx, idx_2, IN_1d);
        // cout << j << " | i : " <<  i << " | " << (i%4) << " | pref : " << prefix 
        //      << " | set idex : " << set_idx << " | nnz : " << nnz << endl;
        nnz = 0; set_idx = 0; prefix = 0; idx_2.clear();
    }
    // this condition "((i%4) == (Ih%4) - 1)" add as we wait until the region outside the remainder 
    // hopfully it may have a pattern to 3 digits or not
    else if ((i > (Ih - (Ih%4))) && ((i%4) == (Ih%4) - 1))
    {
        // prefix = j*Ih + i;
        // cout << "i >= (Ih - (Ih%4)" << endl;
        CPS_if(Ih, j, i, nnz, prefix, set_idx, idx_2, IN_1d);
        ...
        // cout << j << " | i : " <<  i << " | " << (i%4) << " | pref : " << prefix 
        //      << " | set idex : " << set_idx << " | nnz : " << nnz << endl;
        nnz = 0; set_idx = 0; idx_2.clear();
    }

}


void CPS_OP(MatrixXf& org_mat, int Kh, int Kw, int Oh, int Ow, int Sh, int Sw, int Ih, int Iw,
    vector<int>& ptr, vector<int>& IN_1d, vector<int>& DA_1d, vector<int>& m)
{
    int Ow_1 = (1 + Ow);
    int l_Ow_1;
    ...
    // NPO
    {
        ...
        for (int i = 0; i < Ih; ++i)
        {
            if (org_mat(i, 0) != 0)
            {
                ...
                h = pow(2, i%4);
                set_idx += h;
                if (nnz<3) idx_2.push_back(i);
                CPS_idx(Ih, 0, i, nnz, prefix, set_idx, idx_2, IN_1d);
                DA_1d.push_back(org_mat(i, 0));
                x_helper++;
            } // end if  if (org_mat(i, j) != 0)
            else if ( ((i%4) == 3) || (i > (Ih - (Ih%4))) )
            {
               // cout << " ((i%4) == 3) || (i > (Ih - (Ih%4))) \n" << endl;
               // cout << "0" << " | i : " <<  i << " | " << (i%4) << " | pref : " << prefix 
               //      << " | set idex : " << set_idx << " | nnz : " << nnz << endl;
               CPS_idx(Ih, 0 , i, nnz, prefix, set_idx, idx_2, IN_1d);
            }
        } // end for(i=0; i < Ih; ++i)       
        ...
        {            
            if (org_mat(i, Iw - 1) != 0)
            {
               ...
               h = pow(2, i%4);
               set_idx += h;
               if (nnz < 3) idx_2.push_back((Ih-1)*Ih + i);
               CPS_idx(Ih, (Ih - 1) , i, nnz, prefix, set_idx, idx_2, IN_1d);
    ...
    for (int j = 1; j < Kw - 1; ++j)
    {
        ...
        for (int i = 0; i < Ih; ++i)
        {
            if (org_mat(i, j) != 0)
            {
                ...
                h = pow(2, i%4);
                set_idx += h;
                if (nnz < 3) idx_2.push_back(j*Ih + i);
                CPS_idx(Ih, j , i, nnz, prefix, set_idx, idx_2, IN_1d);
        ...
        for (int i = 0; i < Ih; ++i)
        {
            if (org_mat(i, j_) != 0)
            {
                ...
                h = pow(2, i%4);
                set_idx += h;
                if (nnz < 3) idx_2.push_back(j_*Ih + i);
                CPS_idx(Ih, j_ , i, nnz, prefix, set_idx, idx_2, IN_1d);
...
void CPS_idx_v2(int Ih, int j, int i, int &nnz, int &set_idx,  vector<int>& idx_2, vector<int>& IN_1d)
{
    if ((i%4) == 3)
    {
        // cout << "(i%4) == 3" << endl;
        ...
        else if (nnz < 3)
        {
            // cout << "idx_2 : " << endl;
            // printVector(idx_2);
            IN_1d.insert(IN_1d.end(), idx_2.begin(), idx_2.begin() + nnz); 
        }
        // cout << j << " | i : " <<  i << " | " << (i%4) << " | pref : " << prefix 
        //      << " | set idex : " << set_idx << " | nnz : " << nnz << endl;
        nnz = 0; set_idx = 0; 
...
void CPS_idx_v3(int Ih, int j, int i, int &nnz, int &set_idx,  vector<int>& idx_2, vector<int>& IN_1d)
{
    if (nnz > 2)
    {
        // IN_1d.insert(IN_1d.end(), { j * Ih + i - (i % 4) , set_idx});
        IN_1d.push_back( j * Ih + i - (i % 4));
        IN_1d.push_back( set_idx);
    }
    else if (nnz == 2)
    {
        ...
        // for (int i = 0; i < nnz; ++i)
        // {
        //     IN_1d.push_back(idx_2[i]);
        // }
        IN_1d.push_back(idx_2[0]);
        IN_1d.push_back(idx_2[1]);
    }
    else if (nnz == 1)
    {
        IN_1d.push_back(idx_2[0]);
...
void CPS_OP_v2_(MatrixXf& org_mat, int Kh, int Kw, int Oh, int Ow, int Sh, int Sw, int Ih, int Iw,
    vector<int>& ptr, vector<int>& IN_1d, vector<int>& DA_1d, vector<int>& m, vector<int>& idx_2)
{
    int Ow_1 = (1 + Ow);
    int l_Ow_1;
    int l = 1; // Kw-1  Jump to the second part 
    ...
    Ih_mod_4 = (Ih - (Ih%4));
    {
        ...
        // for (int i = 0; i < Ih; ++i)
        for (int i = 0; i < Ih_mod_4; ++i)
        {
            // nnz_check = (org_mat(i,0) != 0);
            // ind_val = (i * Kw);
            // IN_1d.push_back(ind_val);
#if cpo2cps
            mod_4 = i%4;
...
#if cpo2cps
            // CPS_idx_v2(Ih, 0, i, nnz, set_idx, idx_2, IN_1d);
            if ((mod_4 == 3) && nnz) CPS_idx_v3(Ih, 0, i, nnz, set_idx, idx_2, IN_1d);
#endif
        } // end for(i=0; i < Ih; ++i)
#if cpo2cps_last
        for (int i = Ih_mod_4; i < Ih ; ++i)
        {
#if cpo2cps
            mod_4 = i%4;
#endif
...
#if cpo2cps
        CPS_idx_v3(Ih, 0 , Ih-1, nnz, set_idx, idx_2, IN_1d);
#endif
#endif
        ptr[0] = 0;
        ptr[1] = x_helper;
        ...
        for (int i = 0; i < Ih_mod_4; i++)
        {
            ...
            if (org_mat(i, Iw - 1) != 0 )
            {
                ...
                set_idx += h;
                // if (nnz<3) idx_2.push_back(i);
                idx_2[nnz-1]=(Ih-1)*Ih + i;
#endif
                DA_1d.push_back(org_mat(i, Iw - 1));
                x_helper++;
            }
#if cpo2cps
            // CPS_idx_v2(Ih, (Iw - 1), i, nnz, set_idx, idx_2, IN_1d);
            if ((mod_4 == 3) && nnz) CPS_idx_v3(Ih, (Iw - 1), i, nnz, set_idx, idx_2, IN_1d);
#endif
        } // end for(i = 0; i = Ih; ++i)
#if cpo2cps_last
        for (int i = Ih_mod_4; i < Ih ; ++i)
...
#if cpo2cps
        CPS_idx_v3(Ih, (Iw - 1) , Ih-1, nnz, set_idx, idx_2, IN_1d);
#endif
#endif
        ptr[2] = x_helper;
    }
...
#if cpo2cps
            // CPS_idx_v2(Ih, j, i, nnz, set_idx, idx_2, IN_1d);
            if ((mod_4 == 3) && nnz) CPS_idx_v3(Ih, j, i, nnz, set_idx, idx_2, IN_1d);
#endif
        } // end for(i=0; i < Ih; ++i)
#if cpo2cps_last
        for (int i = Ih_mod_4; i < Ih ; ++i)
...
#if cpo2cps
        CPS_idx_v3(Ih, j , Ih-1, nnz, set_idx, idx_2, IN_1d);
#endif
#endif
        // m_helper no need for it as all the pointers will be incremented by 1
        // ptr_Indexhelper = l * (1 + Ow) - npo_shift + m_helper ( 1 );
...
#if cpo2cps
            // CPS_idx_v2(Ih, j_, i, nnz, set_idx, idx_2, IN_1d);
            if ((mod_4 == 3) && nnz) CPS_idx_v3(Ih, j_, i, nnz, set_idx, idx_2, IN_1d);
#endif
        } // end for(i=0; i < Ih; ++i)
#if cpo2cps_last
        for (int i = Ih_mod_4; i < Ih ; ++i)
...
#if cpo2cps         
        CPS_idx_v3(Ih, j_ , Ih-1, nnz, set_idx, idx_2, IN_1d);
#endif
#endif
        l_Ow_1 = l * Ow_1 - npo_shift;
        ptr[l_Ow_1 + m_helper] = x_helper;
...
void CPS_OP_v2(MatrixXf& org_mat, int Kh, int Kw, int Oh, int Ow, int Sh, int Sw, int Ih, int Iw,
    vector<int>& ptr, vector<int>& IN_1d, vector<int>& DA_1d, vector<int>& m, vector<int>& idx_2)
{
...
void CPS_OP_v3(MatrixXf& org_mat, int Kh, int Kw, int Oh, int Ow, int Sh, int Sw, int Ih, int Iw,
    vector<int>& ptr, vector<int>& IN_1d, vector<int>& DA_1d, vector<int>& m, vector<int>& idx_2)
{
    int Ow_1 = (1 + Ow);
    int l_Ow_1;
    ...
    Ih_mod_4 = (Ih - (Ih%4));
    {
        ...
        for (int i = 0; i < Ih_mod_4; i = i + 4)
        {
            ...
            // CPS_idx_v2(Ih, 0, i, nnz, set_idx, idx_2, IN_1d);
            // if ((mod_4 == 3) && nnz) CPS_idx_v3(Ih, 0, i, nnz, set_idx, idx_2, IN_1d);
            CPS_idx_v3(Ih, 0, i_, nnz, set_idx, idx_2, IN_1d);
        } // end for(i=0; i < Ih; ++i)
        for (int i = Ih_mod_4; i < Ih ; ++i)
        {
            mod_4 = i%4;
        ...
        for (int i = 0; i < Ih_mod_4; i = i + 4)
        {
            ...
            if (org_mat(i_, Iw - 1) != 0 )
            {
                ...
                set_idx = 1;
                // if (nnz<3) idx_2.push_back(i);
                idx_2[nnz-1]=(Ih-1)*Ih + i_;
                DA_1d.push_back(org_mat(i_, (Iw - 1)));
                x_helper++;
            }
            ++i_;
            if (org_mat(i_, Iw - 1) != 0 )
            ...
            // CPS_idx_v2(Ih, 0, i, nnz, set_idx, idx_2, IN_1d);
            // if ((mod_4 == 3) && nnz) CPS_idx_v3(Ih, 0, i, nnz, set_idx, idx_2, IN_1d);
            CPS_idx_v3(Ih, (Iw - 1), i_, nnz, set_idx, idx_2, IN_1d);
        } // end for(i=0; i < Ih; ++i))
    ...
    for (int j = 1; j < Kw - 1; ++j)
    {
        ...
        for (int i = 0; i < Ih_mod_4; i = i + 4)
        {
            ...
            // CPS_idx_v2(Ih, 0, i, nnz, set_idx, idx_2, IN_1d);
            // if ((mod_4 == 3) && nnz) CPS_idx_v3(Ih, 0, i, nnz, set_idx, idx_2, IN_1d);
            CPS_idx_v3(Ih, j, i_, nnz, set_idx, idx_2, IN_1d);
        } // end for(i=0; i < Ih; ++i)
        ...
        for (int i = 0; i < Ih_mod_4; i = i + 4)
        {
            ...
            if (org_mat(i_, j_) != 0 )
            {
                ...
                set_idx = 1;
                // if (nnz<3) idx_2.push_back(i);
                idx_2[nnz-1]=j_*Ih + i_;
                DA_1d.push_back(org_mat(i_, j_));
                x_helper++;
            }
            ++i_;
            if (org_mat(i_, j_) != 0 )
            ...
            // CPS_idx_v2(Ih, 0, i, nnz, set_idx, idx_2, IN_1d);
            // if ((mod_4 == 3) && nnz) CPS_idx_v3(Ih, 0, i, nnz, set_idx, idx_2, IN_1d);
            CPS_idx_v3(Ih, j_, i_, nnz, set_idx, idx_2, IN_1d);
        } // end for(i=0; i < Ih; ++i)
        for (int i = Ih_mod_4; i < Ih ; ++i)
        {
            mod_4 = i%4; 
    ...
    int Kw, int Oh, int Ow, int Sh, int Sw, int Ih, int Iw, std::vector<int>& m, int n)
{
    ...
    if (n != 1)
    {
        ...
        vector<int> idx_2(3, 0);
        CPS_OP_v2(org_fm, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw, ptr_1d, IN_1d, DA_1d, m, idx_2);
    }
    else
    {
        CSR(DA_1d, IN_1d, ptr_1d, org_fm, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw);
    ...
    for (int KK = 0; KK < Kh_list.size(); ++KK)
    {
         ...
         // float density = 0.1;
          ...
          // float density = 0.70;
            ...
            for (; density < 1.05; density += 0.05)
            {
                ...
                float t_cps_creation_V1 = 0;
                float t_cps_creation_V2 = 0;
                float t_cps_creation_v3 = 0;
                ...
                MatrixXf org_fm_T = org_fm.transpose ();
        ...
        // int count_d = ceil(density*Iw*Ih);
            ...
            // CPO V8:
                ...
                std::vector<int> ptr_1d_cps(count_ptr_v8, -1);
                std::vector<int> IN_1d_cps;
                std::vector<int> DA_1d_cps;
                m3_pos = Iw - 2 * Kw + 2;
                {
                    std::vector<int> m_cps(n, m3_pos);
                    for (int k = 0; k < bench_iterations; ++k)
                        // for (int k = 0; k < 1; ++k)
                    {
                        clock_t t_cpo_creation_c;
                        t_cpo_creation_c = clock();
                        // the only thing done here is using of 1d pointer instead of 2d
                        CPS_OP(org_fm, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw, ptr_1d_cps, IN_1d_cps, DA_1d_cps , m_cps);
                        // CPO_EncodingV8(IN_1d_v8, DA_1d_v8, ptr_1d_v8, org_fm, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw, m, n);
                        double elapsed = 1000 * ((double)(clock() - t_cpo_creation_c)) / CLOCKS_PER_SEC; // time in milliseconds
                        if (k > 0)
                            t_cps_creation_V1 += elapsed / (Ih * Iw * 1.0); // normalized timing
                        if (k == bench_iterations - 1)
                        {
                            // cout << "\nCPS 1D Ptr:" << endl;
                            // printVector(ptr_1d_cps);

                            // cout << "\nCPS 1D Index:" << endl;
                            // printVector(IN_1d_cps);

                            // cout << "\nCPS 2D DA:" << endl;
                            // // print2DVector(DA);
                            // printVector(DA_1d_cps);
                            // cout << "\nExiting Encoding V7" << endl;
                        }
                        if (k != bench_iterations - 1)
                        {
                ...
                std::vector<int> ptr_1d_cps_v2(count_ptr_v8, -1);
                std::vector<int> IN_1d_cps_v2;
                std::vector<int> DA_1d_cps_v2;
                m3_pos = Iw - 2 * Kw + 2;
                vector<int> idx_2(3, 0);
                std::vector<int> m_cps(n, m3_pos);
                {
                        ...
                        // for (int k = 0; k < 1; ++k)
                    {
                        ...
                        t_cpo_creation_c = clock();
                        // the only thing done here is using of 1d pointer instead of 2d
                        CPS_OP_v2(org_fm, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw, ptr_1d_cps_v2, IN_1d_cps_v2, DA_1d_cps_v2 , m_cps, idx_2);
                        // CPO_EncodingV8(IN_1d_v8, DA_1d_v8, ptr_1d_v8, org_fm, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw, m, n);
                        double elapsed = 1000 * ((double)(clock() - t_cpo_creation_c)) / CLOCKS_PER_SEC; // time in milliseconds
                        if (k > 0)
                            t_cps_creation_V2 += elapsed / (Ih * Iw * 1.0); // normalized timing
                        if (k == bench_iterations - 1)
                        {
                            // cout << "\nCPS 1D Ptr:" << endl;
                            // printVector(ptr_1d_cps_v2);

                            // cout << "\nCPS 1D Index:" << endl;
                            // printVector(IN_1d_cps_v2);

                            // cout << "\nCPS 2D DA:" << endl;
                            // // print2DVector(DA);
                            // printVector(DA_1d_cps_v2);
                            // cout << "\nExiting Encoding V7" << endl;
                        }
                        if (k != bench_iterations - 1)
                        {
                ...
                std::vector<int> ptr_1d_cps_v3(count_ptr_v8, -1);
                std::vector<int> IN_1d_cps_v3;
                std::vector<int> DA_1d_cps_v3;
                m3_pos = Iw - 2 * Kw + 2;
                vector<int> idx_3(3, 0);
                {
                    std::vector<int> m_cps(n, m3_pos);
                    for (int k = 0; k < bench_iterations; ++k)
                        // for (int k = 0; k < 1; ++k)
                    {
                        ...
                        t_cpo_creation_c = clock();
                        // the only thing done here is using of 1d pointer instead of 2d
                        CPS_OP_v3(org_fm, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw, ptr_1d_cps_v3, IN_1d_cps_v3, DA_1d_cps_v3 , m_cps, idx_3);
                        // CPO_EncodingV8(IN_1d_v8, DA_1d_v8, ptr_1d_v8, org_fm, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw, m, n);
                        double elapsed = 1000 * ((double)(clock() - t_cpo_creation_c)) / CLOCKS_PER_SEC; // time in milliseconds
                        if (k > 0)
                            t_cps_creation_v3 += elapsed / (Ih * Iw * 1.0); // normalized timing
                        if (k == bench_iterations - 1)
                        {
                            // cout << "\nCPS 1D Ptr:" << endl;
                            // printVector(ptr_1d_cps_v3);

                            // cout << "\nCPS 1D Index:" << endl;
                            // printVector(IN_1d_cps_v3);

                            // cout << "\nCPS 2D DA:" << endl;
                            // // print2DVector(DA);
                            // printVector(DA_1d_cps_v3);
                            // cout << "\nExiting Encoding V7" << endl;
                        }
                        if (k != bench_iterations - 1)
                        {
                ...
                bool s_cscc_vs_cpsV2 = (t_cscc_creation > t_cps_creation_V2);
                bool s_cpo_vs_cspV2 = (t_cpo_creation_V8 >= t_cps_creation_V2);
                bool s_cscc_vs_cpsV3 = (t_cscc_creation > t_cps_creation_v3);
                bool s_cpo_vs_cpsV3 = (t_cpo_creation_V8 > t_cps_creation_v3);
                bool s_cpsV2_vs_cpsv3 = (t_cps_creation_V2 > t_cps_creation_v3);
                // Index check CPS
                int cps_check = check2Vectors(IN_1d_cps , IN_1d_cps_v3);
                // cout << "Check the same generated index : " << cps_check << endl;
               
                cout << density << "\t" << density_cal << "\t" << Kh << "\t" << Kw << "\t" << Ih << "\t" << Iw << "\n"
                    << "im2col: " << t_im2col_creation 
                    << "\tt_cscc: " << t_cscc_creation 
                    // << "\tt_cpo_creation: " << t_cpo_creation
                    //<< "\tt_cpo_creation_V6: " << t_cpo_creation_woRep 
                    // << "\tt_cpo_creationV7: " << t_cpo_creation_V7 
                    << "\tt_cpo_enc_V8: " << t_cpo_creation_V8 
                    << "\tt_cps_enc_V1: " << t_cps_creation_V1 
                    << "\tt_cps_enc_V2: " << t_cps_creation_V2 
                    << "\tt_cps_enc_V3: " << t_cps_creation_v3
                    << "\tcpo_>=_cspsV2: " << s_cpo_vs_cspV2
                    << "\ncscc_vs_cpsv2: "      << s_cscc_vs_cpsV2
                    << "\tcscc_vs_cpsv3: "      << s_cscc_vs_cpsV3
                    << "\ncpo_vs_cpsv3: "       << s_cpo_vs_cpsV3
                    << "\tcpsV2_vs_cpsv3: "       << s_cpsV2_vs_cpsv3
                    << "\n\n***Index_check: "      << cps_check
                    << endl << endl;
                // exit(0);
