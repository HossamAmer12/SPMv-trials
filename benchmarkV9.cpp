      ...
      {    
       ...
       // cout << "\n" << type_ptr << " ==> Current Submat " << submat << ", ptr:  " <<  x <<  ", ptr+1: " << ptr[type_ptr][submat+1]  << endl;  
        ...
        for(int l = 0; l < Kh; ++l)
        {
            ...
            if(y_out >= 0 && y_out < Oh) {
               O[y_out][x_out] += used_data * K[input_index%Kw + l*Kw];
               O[y_out][x_out + 1] += used_data * K[kernel_common_index - 1]; 
               

               for(int i = 2; i <= type_ptr; ++i)
               {
...
//                }
        ...
        // V9 Remove repeats and transofrm 2d to 1d:
      int count_ptr_v9 = 0;
      for(int i = 0 ; i < ptr.size(); ++i)
      {
      ...
      ///////////
...

        {
            ...
            if(n != 1)
            {
                ...
                // copy data to double
                std::vector<double> DA_1d_v9_d(DA_1d_v9.size(), 0);

                for(int h = 0; h < DA_1d_v9.size(); ++h)
                {
                  DA_1d_v9_d[h] = DA_1d_v9[h];
                }
                ...
                for(int k=0;k<bench_iterations;k++){
                  ...
                  conv_CPO_v9_trim(O_v9, Kernel, IN_1d_v9, DA_1d_v9_d, ptr_1d_v9, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw);
                  // conv_CPO_v8_trim(O_v9, Kernel, IN_1d_v9, DA_1d_v9_d, ptr_1d_v9, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw);
                  ...
                  double elapsed  = 1000*((double)(clock()-t6))/CLOCKS_PER_SEC; // time in milliseconds
                  if(k > 0)
                      t_cpoV9 += elapsed/(Ih*Iw*1.0); // normalized timing
              } // end k lop 
            ...
            t5 = clock();
              ...
              conv_CPO_v7(O_v9, Kernel, IN_1d,  DA_1d_v7_d, ptr_1d, Kh, Kw, Oh, Ow, Sh, Sw, Ih, Iw);
...
//                }
        ...
        <<"\tim2col\t"<<t_im2col<<"\tcsr\t" <<t_csr <<"\tcpoV5\t"<< t_cpoV5 <<"\tcpoV6\t"<< t_cpoV6 <<"\tcpoV8\t"<< t_cpoV8 <<"\tcpoV9\t"<< t_cpoV9
        ...
        <<"\tpercentV8\t"<< 100.0*(t_im2col-t_cpoV8)/t_im2col  <<"\tpercentV9\t"<< 100.0*(t_im2col-t_cpoV9)/t_im2col << "\t" << s << "\n";
        ...
        // <<"\tpercentV7\t"<< 100.0*(t_im2col-t_cpoV7)/t_im2col  <<"\tpercentV9\t"<< 100.0*(t_im2col-t_cpoV9)/t_im2col << "\t" << s << "\n";
         ...
         myfile << "B-" << bench_iterations << "\t" << Kh << "x" << Kw  << " | " <<  Ih << "x" << Iw <<  ") batch\t"<<1
        <<"\ttarget_density\t"<<density<<"\tdensity\t"<<density_cal
        <<"\tim2col\t"<<t_im2col<<"\tcsr\t" <<t_csr <<"\tcpoV5\t"<< t_cpoV5 <<"\tcpoV8\t"<< t_cpoV8 <<"\tcpoV9\t"<< t_cpoV9
        <<"\tpercentCSSC\t"<< 100.0*(t_im2col-t_csr)/t_im2col  <<"\tpercentV5\t"<< 100.0*(t_im2col-t_cpoV5)/t_im2col <<"\tpercentV6\t"<< 100.0*(t_im2col-t_cpoV6)/t_im2col  
        <<"\tpercentV8\t"<< 100.0*(t_im2col-t_cpoV8)/t_im2col  <<"\tpercentV9\t"<< 100.0*(t_im2col-t_cpoV9)/t_im2col << "\t" << s << "\n";
