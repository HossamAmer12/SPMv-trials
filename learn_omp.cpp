...
#pragma omp parallel for default(none) private(i) shared(x, n)
 for( i = 0; i < n ; ++i)
 {
   x.push_back(omp_get_thread_num());  
 }
 ...
 VectorXf filter_vectorized  = VectorXf::Ones(Kh*Kw);
  ...
  Aindptr.push_back(nz);
   ...
   {  
      ...
      int num_threads_  = Aindptr.size(); 
      int piece_size    = ceil(1.0*Aindptr.size() / num_threads_);
      // int p;

#pragma omp parallel num_threads(num_threads_)
{
   int p;
//  #pragma omp for private(p) schedule(static, chunk)
#pragma omp parallel for default(none) private(p) shared(d_o2, filter_vectorized, Adata, Aindices, Aindptr, Kh, Kw, Oh, Ow, piece_size)
    
   // p = omp_get_thread_num();
    for(p = 0; p < Ow; p = p + piece_size) 
    {
